...
const hurryUpStyleSelect = document.getElementById("hurryUpStyleSelect");
const hurryUpSelect = document.getElementById("hurryUpSelect");
const hurryUpDesc = document.getElementById("hurryUpDesc");
...
const HURRY_FLASH_COLOR = "#ff5555";
...
const settings = {
  h24: loadBool("timer_seg_h24", true),
  leading: loadBool("timer_seg_leading", true),
  auto: loadBool("timer_seg_auto", false),
  hours: loadBool("timer_seg_hours", true),
  minutes: loadBool("timer_seg_minutes", true),
  seconds: loadBool("timer_seg_seconds", true),
  ticks: loadBool("timer_seg_ticks", true),
  italic: loadBool("timer_seg_italic", false),

  tps: loadNum("timer_seg_tps", 40),

  preset: localStorage.getItem("timer_seg_preset") || "default",
  corners: localStorage.getItem("timer_seg_corners") || "chop",
  segmentHeight: localStorage.getItem("timer_seg_segment_height") || "normal",
  symmetry: loadBool("timer_seg_symmetry", true),

  flashSeparators: loadBool("timer_seg_flash_separators", true),
  flashEvery: loadNum("timer_seg_flash_every", 0.5),
  evenFlashing: loadBool("timer_seg_even_flashing", false),

  activeColor: localStorage.getItem("timer_seg_active_color") || "#ffffff",
  backgroundColor: localStorage.getItem("timer_seg_background_color") || "#000000",
  inactiveEnabled: loadBool("timer_seg_inactive_enabled", true),

  sevenStyle: localStorage.getItem("timer_seg_seven_style") || "default",
  sixStyle: localStorage.getItem("timer_seg_six_style") || "default",
  nineStyle: localStorage.getItem("timer_seg_nine_style") || "default",

  customDigitSvg: localStorage.getItem("timer_seg_custom_digit_svg") || "",
  customDigitName: localStorage.getItem("timer_seg_custom_digit_name") || "",

  direction: localStorage.getItem("timer_seg_direction") || "from",
  initialTicks: loadNum("timer_seg_initial_ticks", 9 * 60 * 40 + 59 * 40 + 39),
  remainingTicks: loadNum("timer_seg_remaining_ticks", 9 * 60 * 40 + 59 * 40 + 39),
  timeTargetLabel: localStorage.getItem("timer_seg_time_target_label") || "",
  timeTargetAt: loadNum("timer_seg_time_target_at", 0),
  running: false,
  lastMs: 0,
  flashSpeed: localStorage.getItem("timer_seg_hurry_flash_speed") || "off",
  flashValue: localStorage.getItem("timer_seg_hurry_flash_value") || "",
  flashPhase: false,
  flashTimer: null,
  flashDelay: 333,
  ...
  musicUploadName: localStorage.getItem("timer_seg_music_upload_name") || "",
  musicPaused: loadBool("timer_seg_music_paused", false),
  musicVolume: loadNum("timer_seg_music_volume", 1),
  musicStatus: localStorage.getItem("timer_seg_music_status") || "unknown",
  ytId: localStorage.getItem("timer_seg_yt_id") || "",
  ytReady: false,
  yt: null,
...
const HURRY_UP_PRESETS = {
  ...
  mario: {
    label: "Mario",
    sub: [
      { value: "../standard/hurryup/Mario/hurryup-smbnes", label: "Super Mario Bros - NES", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Mario/hurryup-smbgen", label: "Super Mario - Genesis", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Mario/hurryup-smb3", label: "Super Mario Bros 3", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Mario/hurryup-smw", label: "Super Mario World", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Mario/hurryup-nsmb", label: "New Super Mario Bros", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Mario/hurryup-sm3d", label: "Super Mario 3D Land", desc: "Plays at Hurry Up start." }
    ]
  },
  sonic: {
    label: "Sonic",
    sub: [
      { value: "../standard/hurryup/Sonic/hurryup-soniclw", label: "Sonic Lost World", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-GreenHillFinal", label: "Sonic Rumble - Green Hill", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-ChemicalPlantFinal", label: "Sonic Rumble - Chemical Plant", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-SkySanctuaryFinal", label: "Sonic Rumble - Sky Sanctuary", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-SeasideHillFinal", label: "Sonic Rumble - Seaside Hill", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-FrozenFactoryFinal", label: "Sonic Rumble - Frozen Factory", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-LavaMountainFinal", label: "Sonic Rumble - Lava Mountain", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-SilentForestFinal", label: "Sonic Rumble - Silent Forest", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-StarlightCarnivalFinal", label: "Sonic Rumble - Starlight Carnival", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-PinballCarnivalFinal", label: "Sonic Rumble - Pinball Carnival", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-DesertRuinsFinal", label: "Sonic Rumble - Desert Ruins", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-SweetMountainFinal", label: "Sonic Rumble - Sweet Mountain", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-CityEscapeFinal", label: "Sonic Rumble - City Escape", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-SpaceColonyARKFinal", label: "Sonic Rumble - Space Colony ARK", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-PlanetWispFinal", label: "Sonic Rumble - Planet Wisp", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-PopFineFinal", label: "Sonic Rumble - Pop Fine", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-FantasyAdventureFinal", label: "Sonic Rumble - Fantasy Adventure", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-FancyCuteFinal", label: "Sonic Rumble - Fancy Cute", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Sonic/hurryup-sonicrumble-ShibuyaCrossingFinal", label: "Sonic Rumble - Shibuya Crossing", desc: "Plays at Hurry Up start." }
    ]
  },
  ggd: {
  ...
  splatoon: {
    label: "Splatoon",
    sub: [
      { value: "../standard/hurryup/Splatoon/now_or_never-1.mp3", label: "Splatoon 1 - Now or Never!", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Splatoon/now_or_never-1bDanFourts.mp3", label: "Splatoon 1 Beta - Dan Fourts", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Splatoon/now_or_never-1e.mp3", label: "Splatoon 1 Event - Now or Never!", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Splatoon/now_or_never-2.mp3", label: "Splatoon 2 - Now or Never!", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Splatoon/now_or_never-2d.mp3", label: "Splatoon 2 Demo - Now or Never!", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Splatoon/now_or_never-2e.mp3", label: "Splatoon 2 Event - Now or Never!", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Splatoon/now_or_never-2f.mp3", label: "Splatoon 2 Final Fest - Now or Never!", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Splatoon/now_or_never-3.mp3", label: "Splatoon 3 - Now or Never!", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Splatoon/now_or_never-3e.mp3", label: "Splatoon 3 Event - Now or Never!", desc: "Plays at Hurry Up start." },
      { value: "../standard/hurryup/Splatoon/now_or_never-3c.mp3", label: "Splatoon 3 TableTurf - Now or Never!", desc: "Plays at Hurry Up start." }
    ]
  }
};

const MARIO_HURRY_IDS = new Set([
  "../standard/hurryup/Mario/hurryup-smbnes",
  "../standard/hurryup/Mario/hurryup-smbgen",
  "../standard/hurryup/Mario/hurryup-smb3",
  "../standard/hurryup/Mario/hurryup-smw",
  "../standard/hurryup/Mario/hurryup-nsmb",
  "../standard/hurryup/Mario/hurryup-sm3d"
]);
...
function saveSettings() {
  markDisplayDirty?.();
  localStorage.setItem("timer_seg_h24", settings.h24 ? "1" : "0");
  localStorage.setItem("timer_seg_leading", settings.leading ? "1" : "0");
  localStorage.setItem("timer_seg_auto", settings.auto ? "1" : "0");
  localStorage.setItem("timer_seg_hours", settings.hours ? "1" : "0");
  localStorage.setItem("timer_seg_minutes", settings.minutes ? "1" : "0");
  localStorage.setItem("timer_seg_seconds", settings.seconds ? "1" : "0");
  localStorage.setItem("timer_seg_ticks", settings.ticks ? "1" : "0");
  localStorage.setItem("timer_seg_italic", settings.italic ? "1" : "0");

  localStorage.setItem("timer_seg_tps", String(settings.tps));
  localStorage.setItem("timer_seg_preset", settings.preset);
  localStorage.setItem("timer_seg_corners", settings.corners);
  localStorage.setItem("timer_seg_segment_height", settings.segmentHeight);
  localStorage.setItem("timer_seg_symmetry", settings.symmetry ? "1" : "0");

  localStorage.setItem("timer_seg_flash_separators", settings.flashSeparators ? "1" : "0");
  localStorage.setItem("timer_seg_flash_every", String(settings.flashEvery));
  localStorage.setItem("timer_seg_even_flashing", settings.evenFlashing ? "1" : "0");

  localStorage.setItem("timer_seg_active_color", settings.activeColor);
  localStorage.setItem("timer_seg_background_color", settings.backgroundColor);
  localStorage.setItem("timer_seg_inactive_enabled", settings.inactiveEnabled ? "1" : "0");

  localStorage.setItem("timer_seg_seven_style", settings.sevenStyle);
  localStorage.setItem("timer_seg_six_style", settings.sixStyle);
  localStorage.setItem("timer_seg_nine_style", settings.nineStyle);
  localStorage.setItem("timer_seg_custom_digit_svg", settings.customDigitSvg || "");
  localStorage.setItem("timer_seg_custom_digit_name", settings.customDigitName || "");
  localStorage.setItem("timer_seg_direction", settings.direction);
  localStorage.setItem("timer_seg_initial_ticks", String(settings.initialTicks));
  localStorage.setItem("timer_seg_remaining_ticks", String(settings.remainingTicks));
  localStorage.setItem("timer_seg_time_target_label", settings.timeTargetLabel || "");
  ...
  localStorage.setItem("timer_seg_music_url", settings.musicUrl || "");
  localStorage.setItem("timer_seg_music_source", settings.musicSource || "url");
  localStorage.setItem("timer_seg_music_upload_name", settings.musicUploadName || "");
  localStorage.setItem("timer_seg_music_paused", settings.musicPaused ? "1" : "0");
...
function getClockStyleKey() {
  return JSON.stringify({
    ...
    activeColor: getRenderedActiveColor(),
...
function setColorEditorMode(mode) {
  ...
  localStorage.setItem("timer_timer_seg_color_editor_mode", mode);
...
function isSplatoonNowOrNever(value = settings.hurryUp) {
  return /\/hurryup\/Splatoon\/now_or_never-/i.test(String(value || ""));
}

function isSplatoonCardsNowOrNever(value = settings.hurryUp) {
  return /\/hurryup\/Splatoon\/now_or_never-3c\.mp3(?:[?#].*)?$/i.test(String(value || ""));
}

function isSplatoonFixedLastMinute(value = settings.hurryUp) {
  return isSplatoonNowOrNever(value) && !isSplatoonCardsNowOrNever(value);
}

function getHurryVolume() {
  const vol = window.GSGlobal?.getEffectiveAudioVolume?.("hurry");
  return Number.isFinite(vol) ? vol : 1;
}
...
function populateHurryUpTracks({ keepValue = true } = {}) {
  if (!hurryUpStyleSelect || !hurryUpSelect) return;
  const style = HURRY_UP_PRESETS[settings.hurryUpStyle] ? settings.hurryUpStyle : "none";
  settings.hurryUpStyle = style;
  hurryUpStyleSelect.value = style;

  const group = HURRY_UP_PRESETS[style] || HURRY_UP_PRESETS.none;
  const previous = keepValue ? settings.hurryUp : "";
  hurryUpSelect.innerHTML = "";
  group.sub.forEach(track => {
    const option = document.createElement("option");
    option.value = track.value;
    option.textContent = track.label;
    hurryUpSelect.appendChild(option);
  });

  if (previous && group.sub.some(track => track.value === previous)) hurryUpSelect.value = previous;
  else hurryUpSelect.value = group.sub[0]?.value || "";

  settings.hurryUp = hurryUpSelect.value;
  const selected = group.sub.find(track => track.value === settings.hurryUp) || group.sub[0];
  if (hurryUpDesc) hurryUpDesc.textContent = selected?.desc || "";
...
function getMusicVolume() {
  const local = Number.isFinite(settings.musicVolume) ? settings.musicVolume : 1;
  const global = window.GSGlobal?.getEffectiveAudioVolume?.("music") ?? 1;
...
function applyMusicVolume() {
  ...
  if (settings.yt && settings.ytReady) {
    try { settings.yt.setVolume(Math.round(effective * 100)); } catch {}
  }
}

function setMusicStatus(status, label = "") {
  settings.musicStatus = status;
  const text = {
    unknown: "No music",
    loading: "Loading music...",
    saved: "Music saved",
    uploaded: "Uploaded audio saved",
    invalid: "Invalid music URL or file"
  }[status] || "No music";
  if (musicStatusText) musicStatusText.textContent = label || text;
}

function syncMusicPauseButton() {
  if (!musicPauseBtn) return;
  musicPauseBtn.textContent = settings.musicPaused ? "Unpause Music" : "Pause Music";
  musicPauseBtn.setAttribute("aria-pressed", settings.musicPaused ? "true" : "false");
}

function syncMusicVolumeControl() {
  if (!musicVolume) return;
  const pct = clamp(Math.round((Number.isFinite(settings.musicVolume) ? settings.musicVolume : 1) * 100), 0, 100);
...
function stopAllBGM() {
  ...
  if (settings.yt && settings.ytReady) {
    try { settings.yt.pauseVideo(); } catch {}
  }
}
...
function stopBGMAndReset() {
  ...
  if (settings.yt && settings.ytReady) {
    try {
      settings.yt.setPlaybackRate(1);
      settings.yt.seekTo(0, true);
      settings.yt.pauseVideo();
    } catch {}
  }
}
...
function setMusicPlaybackRate(rate) {
  ...
  if (bgAudio && bgAudio.src) {
    try { bgAudio.playbackRate = nextRate; } catch {}
  }
  if (settings.yt && settings.ytReady) {
    try { settings.yt.setPlaybackRate(nextRate); } catch {}
  }
}

function syncHurryMusicSpeed() {
  const shouldSpeedUp = settings.running
    && settings.hurryUp === "__speedup__"
    && timeLeftTicks() <= hurryFlashTrigger();
  setMusicPlaybackRate(shouldSpeedUp ? 1.25 : 1);
}
...
function playBGM() {
  ...
  if (settings.yt && settings.ytReady) {
    try { settings.yt.playVideo(); } catch {}
  } else if (settings.ytId) {
    settings.ytAutoplayWhenReady = true;
  }
}
...
window.onYouTubeIframeAPIReady = function onYouTubeIframeAPIReady() {
  try { if (typeof previousYTReady === "function") previousYTReady(); } catch {}
  settings.ytReady = true;
  if (settings.ytId) ensureYT(settings.ytId);
};
...
function ensureYT(id) {
  ...
  if (!window.YT || !window.YT.Player) {
    settings.ytAutoplayWhenReady = settings.running;
    return;
  }
  if (settings.yt) {
    try { settings.yt.loadVideoById(id); } catch {}
    return;
  }
  settings.yt = new YT.Player("ytPlayer", {
    width: "100%",
    height: "100%",
    videoId: id,
    ...
    events: {
      onReady: () => {
        settings.ytReady = true;
        applyMusicVolume();
        if (settings.running || settings.ytAutoplayWhenReady) {
          settings.ytAutoplayWhenReady = false;
          playBGM();
        }
      },
      onStateChange: (event) => {
        try {
          if (event.data === YT.PlayerState.ENDED && settings.running && !settings.musicPaused) {
            settings.yt.seekTo(0, true);
            settings.yt.playVideo();
          }
        } catch {}
      }
    }
...
function clearYT() {
  if (settings.yt) {
    try { settings.yt.destroy(); } catch {}
    settings.yt = null;
  }
  settings.ytId = "";
  settings.ytReady = false;
...
const UPLOAD_AUDIO_DB = "mimi-clock-segmented-timer-audio";
...
function setUploadedMusicSource(url, name) {
  ...
  try {
    ...
    applyMusicVolume();
    if (settings.running) playBGM();
  } catch {}
  settings.musicUrl = "";
  settings.ytId = "";
  settings.musicSource = "upload";
  settings.musicUploadName = name || "Uploaded audio";
  if (musicUrlInput) musicUrlInput.value = settings.musicUploadName;
  setMusicStatus("uploaded", `Uploaded: ${settings.musicUploadName}`);
  saveSettings();
}
...
async function restoreUploadedMusic() {
  ...
  try {
    ...
    if (!stored?.blob) {
      settings.musicSource = "url";
      settings.musicUploadName = "";
    ...
    setUploadedMusicSource(uploadedMusicObjectUrl, stored.name || settings.musicUploadName);
  } catch {
...
function applyMusicUrl(url) {
  ...
  if (ytId) {
    ...
    settings.ytId = ytId;
    ensureYT(ytId);
    setMusicStatus("saved", "YouTube music saved");
    saveSettings();
    return;
  }

  if (looksLikeDirectAudio(u) && bgAudio) {
    clearYT();
    try {
      bgAudio.src = u;
      bgAudio.loop = true;
    ...
    setMusicStatus("saved", "Audio URL saved");
    saveSettings();
    return;
  }
...
function hurryFlashTrigger() {
  if (isSplatoonFixedLastMinute()) return 60 * settings.tps;
  const parsed = parseDurationToTicks(settings.flashValue);
  return Number.isFinite(parsed) ? parsed : 60 * settings.tps;
}

function updateHurryFlashControls() {
  if (!flashValueInput) return;
...
function maybeTriggerHurryUp() {
  if (!settings.running || settings.hurryUpPlayed || !settings.hurryUp) return;
  if (timeLeftTicks() > hurryFlashTrigger()) return;
  settings.hurryUpPlayed = true;
  doHurryUp();
}

async function doHurryUp() {
  const basePath = String(settings.hurryUp || "").trim();
  if (!basePath) return;
  ...
  const wasBGMPlaying =
    ...
    (settings.yt && settings.ytReady && (() => {
      try { return settings.yt.getPlayerState() === YT.PlayerState.PLAYING; }
      catch { return false; }
    })());
  ...
  const isMario = MARIO_HURRY_IDS.has(basePath) || MARIO_HURRY_IDS.has(basePath.replace(/^.*\/(hurryup-[^/]+)$/, "../standard/hurryup/Mario/$1"));
  const isSplatoonFinisher = isSplatoonFixedLastMinute(basePath);
  audio.loop = !(isMario || isGGDShip || isSplatoonFinisher);
  settings.hurryUpAudio = audio;

  try { await audio.play(); }
  catch {
    if (settings.hurryUpAudio === audio) settings.hurryUpAudio = null;
    return;
  }
  ...
  if (isMario) {
    audio.addEventListener("ended", () => {
      if (!settings.running) return;
      setMusicPlaybackRate(1.25);
      if (settings.yt && settings.ytReady) {
        try { settings.yt.seekTo(0, true); settings.yt.playVideo(); } catch {}
      } else if (bgAudio && bgAudio.src) {
        try { bgAudio.currentTime = 0; bgAudio.play(); } catch {}
      }
    }, { once: true });
  }
}
...
function syncInputsFromSettings() {
  ...
  setValue("hurryUpStyleSelect", settings.hurryUpStyle);
  populateHurryUpTracks({ keepValue: true });
  if (musicUrlInput) musicUrlInput.value = settings.musicSource === "upload" ? settings.musicUploadName : settings.musicUrl;
  syncMusicPauseButton();
  syncMusicVolumeControl();
  setMusicStatus(settings.musicStatus, settings.musicSource === "upload" && settings.musicUploadName ? `Uploaded: ${settings.musicUploadName}` : "");
  optPreset.value = settings.preset;
...
hurryUpStyleSelect?.addEventListener("change", () => {
  settings.hurryUpStyle = hurryUpStyleSelect.value;
  settings.hurryUpPlayed = false;
  stopHurryAudio({ force: true });
  populateHurryUpTracks({ keepValue: false });
  syncHurryMusicSpeed();
  if (settings.running) maybeTriggerHurryUp();
  saveSettings();
});
...
hurryUpSelect?.addEventListener("change", () => {
  ...
  settings.hurryUpPlayed = false;
  stopHurryAudio({ force: true });
  const group = HURRY_UP_PRESETS[settings.hurryUpStyle] || HURRY_UP_PRESETS.none;
  const selected = group.sub.find(track => track.value === settings.hurryUp) || group.sub[0];
  if (hurryUpDesc) hurryUpDesc.textContent = selected?.desc || "";
  updateHurryFlashControls();
  syncHurryMusicSpeed();
...
musicUrlSubmitBtn?.addEventListener("click", event => {
  event.preventDefault();
  applyMusicUrl(musicUrlInput ? musicUrlInput.value : "");
});

musicUrlInput?.addEventListener("keydown", event => {
  if (event.key === "Enter") {
    event.preventDefault();
...
musicPauseBtn?.addEventListener("click", event => {
  event.preventDefault();
  settings.musicPaused = !settings.musicPaused;
  syncMusicPauseButton();
  if (settings.musicPaused) stopAllBGM();
  else playBGM();
  saveSettings();
});
...
musicVolume?.addEventListener("input", event => {
  ...
  window.GSGlobal?.playSliderSound?.("music", settings.musicVolume);
  saveSettings();
});
...
musicUploadBtn?.addEventListener("click", event => {
  event.preventDefault();
...
(async function init() {
  ...
  if (settings.musicSource === "upload") {
    if (musicUrlInput) musicUrlInput.value = settings.musicUploadName || "Uploaded audio";
    restoreUploadedMusic();
  } else if (settings.musicUrl) {
    if (musicUrlInput) musicUrlInput.value = settings.musicUrl;
    applyMusicUrl(settings.musicUrl);
  } else {
    setMusicStatus("unknown");
  }
  syncMusicPauseButton();
